  ...
  class Assets
    ...
    module Helpers
      ...
      def _subresource_integrity?
        Container['assets'].subresource_integrity?
      end

      # @api private
      def _subresource_integrity_value(source, ext)
        source = "#{source}#{ext}" unless source =~ /#{Regexp.escape(ext)}\z/
        Container['assets'][source].subresource_integrity_value unless _absolute_url?(source)
      end
      ...
      def _crossorigin?(source)
        ...
        Container['assets'].crossorigin?(source)
      end

      # @since 0.1.0
      # @api private
      def _relative_url(source)
        Container['assets'][source].path
      end

      # @since 0.1.0
      # @api private
      def _absolute_url(source)
        Container['assets'][source].url
      end
